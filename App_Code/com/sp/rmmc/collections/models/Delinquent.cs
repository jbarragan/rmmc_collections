...
"(select top 1 ms_loan_balance.unapplied_bal from ms_loan_balance where ms_loan_balance.loan_id = base.loan_id order by ms_loan_balance.unapplied_bal ) as unapplied_bal, " +
"(select sum(hist.pmt_amt) from ms_loan_history hist where hist.loan_id = base.loan_id and hist.trans_type_cd in ('REG', 'REGR') and hist.paid_dt >=  dateadd(dd, -30, base.misc_dt_value)) as last_payments_balance, " +
"(select top 1 ms_loan_balance.late_chrg_due_amt from ms_loan_balance where ms_loan_balance.loan_id = base.loan_id order by ms_loan_balance.late_chrg_due_amt ) as late_chrg_due_amt, " +
...
"where 1 = 1 ";
        ...
        public static string in_three_month_delinquent()
        {
        ...
        public static List<Delinquent> in_three_month_delinquent()
        {            
            return get_list(in_three_month_delinquent);
        }
